...
export default function CongratsDemo() {
  ...
  useEffect(() => {
    ...
    const celebrate = () => {
      ...
      // Second burst from right with a slight delay
      setTimeout(() => {
        confetti({
          particleCount: 200,
          spread: 70,
  ...
  // Scroll animation for icons - trigger on first scroll
  useEffect(() => {
    const handleScroll = () => {
      if (!iconsVisible) {
  ...
  // Mock data
  const mockRoom = {
    title: "Household Chores Discussion",
    check_in_date: "2025-12-15",
    resolution_text: "We agreed to create a weekly chore schedule where we alternate responsibilities. Sarah will handle dishes Monday-Wednesday, Mike Thursday-Saturday. We'll review this arrangement in two weeks and adjust if needed."
  };

  const mockSummaries = {
    user1_name: "Sarah",
    user2_name: "Mike",
    user1_summary: "I feel like I'm doing more than my fair share of household chores...",
    user2_summary: "I think we need a clearer system for dividing up the housework..."
  };

  const downloadTranscript = () => {
    alert("In the real app, this would download a PDF transcript");
  };

  const handleShare = (platform) => {
    alert(`In the real app, this would share via ${platform}`);
  };

  const handleRating = (stars) => {
    setRating(stars);

    // In real app, check if user logged in with Google OAuth
    const isGoogleUser = true; // mockUser.oauth_provider === 'google'
    ...
    if (isGoogleUser) {
      ...
      alert(`Thanks for the ${stars} star rating! Opening Google Reviews where you can share your experience (you're already logged in).`);
    } else {
  ...
  const addToCalendar = () => {
    const checkInDate = new Date(mockRoom.check_in_date);
  ...
  const checkInDate = new Date(mockRoom.check_in_date).toLocaleDateString();
